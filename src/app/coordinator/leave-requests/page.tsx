...
export default async function CoordinatorLeaveRequestsPage() {
  const coordinator = await requireRoleOrRedirect("coordinator");
  const snap = await adminDb
    .collection("leaveRequests")
    .where("supervisorId", "==", coordinator.uid)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Revisa, aprueba o rechaza las solicitudes de los supervisores y del administrador.
        </p>
      </div>
              ...
              <TableRow key={request.id}>
                ...
                <TableCell className="text-right">
                  {request.status === "PENDIENTE_SUPERVISOR" ? (
                    <Button
                      size="sm"
                      render={<Link href={`/coordinator/leave-requests/${request.id}`} />}
                      nativeButton={false}
                    >
                      Revisar
                    </Button>
