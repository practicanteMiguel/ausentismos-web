...
export default async function NewSupervisorLeaveRequestPage() {
  const supervisor = await requireRoleOrRedirect("supervisor");

  const [userSnap, contractSnap, fieldSnap] = await Promise.all([
    adminDb.collection("users").doc(supervisor.uid).get(),
    adminDb.collection("contracts").doc(supervisor.contractId!).get(),
    adminDb.collection("fields").doc(supervisor.fieldId!).get(),
  ]);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <LeaveRequestForm
        ...
        redirectPath="/supervisor/leave-requests/mine"
