...
export default async function SupervisorOwnLeaveRequestsPage() {
  const supervisor = await requireRoleOrRedirect("supervisor");
  const snap = await adminDb
    .collection("leaveRequests")
    .where("employeeId", "==", supervisor.uid)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Mis ausentismos</h1>
        <p className="text-sm text-muted-foreground">Estado de tus propias solicitudes.</p>
      </div>
